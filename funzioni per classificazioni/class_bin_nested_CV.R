...
# It's the same funziton as in my other repository "Nested-CV-binary-classification",
# but with new functions ('ibc' and 'hdtbc', built from scratch and present in this folder) 
# with which we can perform the binary classifications.
...
lapply(c('caret','ranger','glmnet','e1071','MASS'), 
...
clbin_nestkfold <- function(data, tipo = 'elasticnet', kfold = 10, nest_kfold = 10, conf_matr = TRUE, seme = 910, ...){
  ...
  prev_mod <- function(dbtr, dbts, model, ...){
    ...
    if(model == 'lda'){ 
      set.seed(seme)
      lda_tr <- MASS::lda(y ~., data = dbtr)
      return(predict(lda_tr,dbts[,-1])$class) }
    ...
    if(model == 'ibc'){ 
      set.seed(seme)
      return(ibc(y_tr=dbtr[,1],x_tr=dbtr[,-1],x_ts=dbts[,-1], k_nn = 'CV', ...)) }
    
    if(model == 'tbc'){ return(tbc(y_tr=dbtr[,1],x_tr=dbtr[,-1],x_ts=dbts[,-1])) }
